...
class GenerateRecommendationView(APIView):
    ...
    def get(self, request):
        logger.info(f"GenerateRecommendationView called for user: {request.user.username}")
        try:
            user = request.user
            ...
            # Log the error for Railway logs
            logger.error(f"Error in GenerateRecommendationView: {str(e)}")
